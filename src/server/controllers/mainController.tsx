...
import { createStore, RootState } from '../../client/store';
...
const tailTemplate = (req: Request, innerHtml: string, state: RootState): string => {
...
export const mainController = async (req: Request, res: Response) => {
  ...
  const matchedRouteInfo = matchedRoute && matchPath(req.path, matchedRoute);
  const params = matchedRouteInfo?.params;
  if (params && matchedRoute && matchedRoute.loadData) {
    await store.dispatch<any>(matchedRoute.loadData(params));
  }
